...
const Textfield = ({ ...props }) => {
  const labelClass =
    "-z-10 tracking-wide dark:text-yellow-200 font-normal bg-inherit m-4 duration-300 ease-in-out absolute top-0 capitalize text-lg";
  const labelActive =
    "-z-10 tracking-wide dark:text-yellow-200 font-thin bg-inherit m-4 duration-300 ease-in-out absolute capitalize -top-10 text-sm pt-[1px]";

  const testValue = () =>
    (props.value === "" || isUndefined(props.value)) && !props.placeholder
      ? labelClass
      : labelActive;
